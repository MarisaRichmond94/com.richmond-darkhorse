...
public class UnsignedIncidentReportPane extends GridPane{
	...
	public UnsignedIncidentReportPane(Director director,Map<String,IncidentReport> unsignedIncidentReports) {
		...
		for(IncidentReport incidentReport : unsignedIncidentReports.values()) {
			String student = incidentReport.getStudentName();
			String date = incidentReport.getStringDate();
			String time = incidentReport.getStringTime();
			...
			newButton.setOnAction(e -> {
	    			if(newButton.isSelected() == true) {
	    				selectedIncidentReports.put(incidentReport.getDocumentID(),incidentReport);
	    			}else if(newButton.isSelected() == false) {
	    				selectedIncidentReports.remove(incidentReport.getDocumentID());
	    			}
    			});
			int row = 0;
